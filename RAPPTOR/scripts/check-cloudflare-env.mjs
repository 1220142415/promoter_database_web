...
const catalogBinding = d1Blocks.find((block) => /^\s*binding\s*=\s*["']RAPPTOR_DB["']\s*$/m.test(block));
...
if (!catalogBinding || !databaseId || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(databaseId) || /^0{8}-0{4}-0{4}-0{4}-0{12}$/.test(databaseId)) {
  console.error('wrangler.toml must contain a real RAPPTOR_DB D1 binding before a Cloudflare build.');
  process.exit(1);
}
