...
OBSOLETE_PILOT_FILES = ('RAPPTOR-HF-PILOT.md', 'pilot-manifest.tsv')


def parse_args():
    parser = argparse.ArgumentParser(description='Upload a complete RAPPTOR release to a Hugging Face Dataset.')
...
def main():
    ...
    if root_pending:
        ...
        create_commit_with_retry(api, args, operations, f'Add RAPPTOR {args.release} release metadata')
    ...
    for index, batch in enumerate(upload_batches, start=1):
        ...
        create_commit_with_retry(api, args, operations, f'Add RAPPTOR {args.release} genomes {batch[0]} through {batch[-1]}')
    ...
    if obsolete:
        api.delete_files(
            ...
            commit_message=f'Remove superseded RAPPTOR {args.release} pilot markers',
        )
