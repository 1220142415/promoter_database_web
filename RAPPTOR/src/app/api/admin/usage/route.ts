...
export async function GET(request: Request) {
  if (!process.env.RAPPTOR_ANALYTICS_USERNAME || !process.env.RAPPTOR_ANALYTICS_PASSWORD) {
    return new Response('Not found.', { status: 404, headers: { 'Cache-Control': 'no-store' } });
  }

  const database = usageDatabase();
  if (!database) {
    return Response.json({ error: 'RAPPTOR_DB is not bound to this deployment.' }, { status: 503 });
  }
  ...
  return new Response(`${body}\n`, {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="rapptor-usage-${dataset}-${report.startDay}-to-${report.endDay}.csv"`,
    },
  });
}
