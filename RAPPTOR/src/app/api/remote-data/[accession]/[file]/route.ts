...
function cacheKey(request: Request, method: string, range: string | null) {
  ...
  url.searchParams.set('__rapptor_cache_version', '3');
  url.searchParams.set('__rapptor_cache_method', method);
  url.searchParams.set('__rapptor_cache_range', range || 'full');
...
async function cachedResponse(key: Request | null) {
  ...
  try {
    ...
    const status = Number(headers.get('x-rapptor-cached-status') || cached.status);
    headers.delete('x-rapptor-cached-status');
    headers.set('X-RAPPTOR-Cache', 'HIT');
...
async function storeResponse(key: Request | null, response: Response) {
  ...
  try {
    ...
    const headers = new Headers(cached.headers);
    headers.set('x-rapptor-cached-status', String(cached.status));
...
async function serve(request: Request, context: RouteContext, headOnly: boolean) {
  ...
  response.headers.set('X-RAPPTOR-Cache', key ? 'MISS' : 'BYPASS');
  ...
  if (requestedRange) {
    ...
    if (!metadataResponse) {
      ...
      if (metadataResponse?.ok) {
        ...
        if (upstreamLength) metadataHeaders.set('X-RAPPTOR-Logical-Length', upstreamLength);
    ...
    logicalSize = Number(metadataResponse.headers.get('x-rapptor-logical-length') || metadataResponse.headers.get('content-length'));
