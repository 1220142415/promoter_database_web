...
function experimentalAssembly(genome: NonNullable<ReturnType<typeof getCyanobacteriaGenome>>): ExperimentalTssGenome | null {
  const evidence = genome.experimentalEvidence;
  const data = genome.assets.experimentalTss;
  if (!evidence || !data) return null;
  return {
    releaseId: cyanobacteriaRelease.releaseId,
    accession: genome.id,
    organismName: genome.organismName,
    strain: genome.strain,
    assemblyName: genome.id,
    genbankAssemblyAccession: evidence.assemblyAccession,
    defaultLocus: genome.defaultLocus,
    primarySequence: genome.primarySequence,
    ...
    studies: [{
      ...
      pmid: evidence.pmid,
      year: evidence.year,
      recordCount: evidence.observationCount,
