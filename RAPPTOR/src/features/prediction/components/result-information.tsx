import { evaluatedStrandsFromSummary, exportCutoffLabel, type JobSummary } from '../live-result';
...
export default function ResultInformation({ summary, inputName, refName }: {
  ...
  const evaluatedStrands = evaluatedStrandsFromSummary(summary);
  const strandLabel = evaluatedStrands.length === 2 ? 'Both strands' : evaluatedStrands.includes('-') ? 'Reverse strand only' : 'Forward strand only';
  const strandsRecorded = summary.evaluated_strands !== undefined || summary.reverse_complementary !== undefined;
  return <section className={styles.panel} aria-labelledby="prediction-information-heading">
    ...
    <dl className={`${styles.factGrid} ${styles.resultFacts} ${styles.simpleResultFacts}`}>
      ...
      <div><dt>Prediction settings</dt><dd>{strandsRecorded ? strandLabel : 'Strands not recorded'}{focused && summary.score_cutoff !== undefined ? <small>Cutoff: {summary.score_cutoff_operator || '>'} {summary.score_cutoff}</small> : !focused && (summary.peak_calling ? <small>Cutoff: {summary.peak_calling.operator} {summary.peak_calling.cutoff}</small> : <small>Cutoff: {exportCutoffLabel(summary)}</small>)}</dd></div>
