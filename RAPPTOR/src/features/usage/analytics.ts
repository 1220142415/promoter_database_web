// Request helpers for the privacy-preserving usage feature. This module
...
export function readUsageSettings(): UsageSettings {
  const retention = Number(envValue('RAPPTOR_ANALYTICS_RETENTION_DAYS'));
  return {
    enabled: isEnabledValue(envValue('RAPPTOR_ANALYTICS')),
    precision: envValue('RAPPTOR_ANALYTICS_PRECISION')?.toLowerCase() === 'city' ? 'city' : 'country',
    retentionDays: Number.isFinite(retention) && retention > 0 ? Math.floor(retention) : DEFAULT_RETENTION_DAYS,
    trustProxyHeaders: isEnabledValue(envValue('RAPPTOR_ANALYTICS_TRUST_PROXY_HEADERS')),
  };
}
