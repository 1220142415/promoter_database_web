...
test.describe('local Cloudflare D1 analytics integration', () => {
  ...
  test('authenticates reports and records two views as one daily visitor', async ({ browser, request }) => {
    ...
    expect(hiddenResponse.headers()['www-authenticate']).toContain('Basic realm="RAPPTOR usage"');
    ...
    const visitorContext = await browser.newContext({
      ...
      userAgent: `Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/140.0.0.0 Safari/537.36 RAPPTORValidation/${Date.now()}`,
    });
    const visitorPage = await visitorContext.newPage();
    await visitorPage.goto('/');
    await expect(visitorPage.getByRole('heading', { name: 'RAPPTOR' })).toBeVisible();
