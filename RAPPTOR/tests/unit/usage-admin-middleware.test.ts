...
const keys = ['RAPPTOR_ANALYTICS_USERNAME', 'RAPPTOR_ANALYTICS_PASSWORD', 'RAPPTOR_DEMO_USERNAME', 'RAPPTOR_DEMO_PASSWORD'];
...
describe('usage dashboard gate', () => {
  ...
  it('challenges configured deployments with its own realm', () => {
    ...
    expect(response.headers.get('www-authenticate')).toContain('RAPPTOR usage');
  ...
  it('uses the dashboard credentials on admin paths even while the demo gate is active', () => {
    ...
    process.env.RAPPTOR_DEMO_PASSWORD = 'demo-password';
