...
def parse_args():
    parser = argparse.ArgumentParser(description='Resume a packed SeqEdge release upload using the shared JSON plan.')
                ...
                for attempt in range(args.retries + 1):
                    try:
                        result = api.create_commit(
                            repo_id=plan['repo'], repo_type='dataset', revision=plan['revision'],
                            operations=operations, commit_message=f"Upload SeqEdge {plan['release']} {batch['id']}",
