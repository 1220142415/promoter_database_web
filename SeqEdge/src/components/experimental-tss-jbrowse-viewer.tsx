...
import SeqEdgeStrandFeaturePlugin, { DIRECTIONAL_ANNOTATION_RENDERER } from '@/jbrowse/strand-feature-plugin';
...
} from '@/lib/experimental-jbrowse-share';
import { visibleTrackRegion } from '@/lib/track-download';
...
export default function ExperimentalTssJBrowseViewer({
  ...
  const { viewState, trackRegistry, initialWarnings } = useMemo(() => {
    ...
    for (const study of genome.studies) {
      ...
      tracks.push({
        ...
        metadata: {
          ...
          seqEdgeExperimentalDownloads: downloads,
        },
        assemblyNames: [assemblyName],
        type: 'FeatureTrack',
    ...
    const stateTree = createViewState({
      ...
      tracks,
      plugins: [SeqEdgeStrandFeaturePlugin, SeqEdgeExperimentalTssPlugin, SeqEdgeTrackDownloadPlugin],
      defaultSession: {
