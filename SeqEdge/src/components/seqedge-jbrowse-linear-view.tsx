...
const SeqEdgeFeatureDetailsPanel = observer(function SeqEdgeFeatureDetailsPanel({
  session,
  widget,
}: {
  session: WidgetSession;
  ...
  return (
    <Paper
      component="aside"
      className="seqedge-feature-details"
      data-testid="seqedge-feature-details-panel"
      aria-label="Feature details"
      elevation={0}
    >
      <Toolbar className="seqedge-feature-details-header" disableGutters>
        <div className="seqedge-feature-details-heading">
          {HeadingComponent
      ...
      <div className="seqedge-feature-details-content">
        <Suspense fallback={<div className="seqedge-feature-details-loading"><LoadingEllipses /></div>}>
          <Component
            model={widget}
            session={session}
...
const SeqEdgeJBrowseLinearView = observer(function SeqEdgeJBrowseLinearView({
  viewState,
}: {
  viewState: ViewModel;
}) {
  ...
  return (
    <ThemeProvider theme={theme}>
      <div className={`seqedge-jbrowse-layout${detailsWidget ? ' seqedge-jbrowse-layout--details' : ''}`}>
        <div className="seqedge-jbrowse-view">
        ...
        {detailsWidget && <SeqEdgeFeatureDetailsPanel session={session} widget={detailsWidget} />}
      </div>
...
export default SeqEdgeJBrowseLinearView;
