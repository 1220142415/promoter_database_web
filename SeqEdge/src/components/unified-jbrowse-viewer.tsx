...
import SeqEdgeExperimentalTssPlugin, { EXPERIMENTAL_TSS_RENDERER } from '@/jbrowse/experimental-tss-plugin';
  ...
  const { viewState, trackRegistry, initialWarnings, trackLabels } = useMemo(() => {
    ...
    if (prediction?.assets.promoterScoresPlus && prediction.assets.promoterScoresMinus) {
      ...
      tracks.push({
        ...
        metadata: {
          ...
          seqEdgeDownloads: [
            predictionDownload('scores-plus', 'RAPPtor raw scores (+ strand)', plusUrl, false).seqEdgeDownload,
            predictionDownload('scores-minus', 'RAPPtor raw scores (- strand)', minusUrl, false).seqEdgeDownload,
          ],
        },
    ...
    if (prediction) {
      ...
      tracks.push({
        ...
        metadata: {
          ...
          seqEdgeEvidenceType: 'prediction',
    ...
    for (const study of studies) {
      ...
      tracks.push({
        ...
        metadata: {
          seqEdgeEvidenceType: 'experimental_tss',
          seqEdgeStudy: {
    ...
    const stateTree = createViewState({
      assembly: {
        ...
        sequence: {
          ...
          metadata: {
            seqEdgeDownload: prediction
              ? predictionDownload('reference', 'Reference sequence', referenceUrl).seqEdgeDownload
              : {
                  kind: 'reference',
  ...
  return (
    <div className="portal-browser-shell" data-testid="jbrowse-viewer" data-viewer-kind="unified">
      ...
      {referenceFailureMessage
        ...
        : <div className="portal-browser"><SeqEdgeJBrowseLinearView viewState={viewState} /></div>}
    </div>
  );
}
