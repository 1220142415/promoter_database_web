...
async function configuredD1() {
  ...
  try {
    ...
    if (!database) throw new ExperimentalTssCatalogUnavailableError('SEQEDGE_DB is not bound to this deployment.');
    return database;
  } catch (cause) {
    if (cause instanceof ExperimentalTssCatalogUnavailableError) throw cause;
...
export const experimentalTssRepository: ExperimentalTssRepository = {
  async getActiveRelease() {
    ...
    throw new ExperimentalTssCatalogUnavailableError('Set EXPERIMENTAL_TSS_CATALOG_PATH to preview the experimental TSS collection locally.');
  },
  async search(query) {
    const local = configuredLocalRepository();
    ...
    throw new ExperimentalTssCatalogUnavailableError('The experimental TSS collection is not configured.');
  },
  async listGenomes() {
    const local = configuredLocalRepository();
