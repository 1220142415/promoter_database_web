...
import { scheduleUsageCollection } from '@/lib/usage-analytics-store';
...
function adminGate(request: NextRequest) {
  const expectedUsername = process.env.SEQEDGE_ANALYTICS_USERNAME;
  const expectedPassword = process.env.SEQEDGE_ANALYTICS_PASSWORD;
  if (!expectedUsername || !expectedPassword) return notFound();
  return matches(request.headers.get('authorization'), expectedUsername, expectedPassword)
    ? null
    : unauthorized('SeqEdge usage');
}
