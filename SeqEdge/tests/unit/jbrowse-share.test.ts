...
describe('JBrowse share URL codec', () => {
  it('round-trips a versioned view with exact track ordering and heights', () => {
    const url = buildJBrowseShareUrl(
      { origin: 'https://seqedge.example', pathname: '/genomes/GCA_000411415.1' },
      state,
    );

    expect(url).toBe(
      'https://seqedge.example/genomes/GCA_000411415.1?view=1&ref=CP003597.1&center=123456&zoom=0.25&rev=0&tracks=sequence%3A120%2Cscores%3A180%2Cpromoters%3A170%2Cannotation%3A170',
    );
  ...
  it('URL-encodes special reference names instead of interpreting them as parameters', () => {
    ...
    const url = new URL(buildJBrowseShareUrl(
      { origin: 'https://seqedge.example', pathname: '/genomes/GCA_1' },
      { ...state, refName },
    ));
  ...
  it('builds from only the origin and pathname and clears credentials, unknown query, and hash', () => {
    const url = new URL(buildJBrowseShareUrl(
      {
        origin: 'https://user:password@seqedge.example/?secret=yes#private',
        pathname: '/genomes/GCA_000411415.1',
      },
      state,
    ));

    expect(url.origin).toBe('https://seqedge.example');
  ...
  it('does not build links for external schemes or forged track tokens', () => {
    expect(() => buildJBrowseShareUrl(
      { origin: 'blob:https://seqedge.example/id', pathname: '/genomes/GCA_1' },
      state,
    )).toThrow(/HTTP/);
    expect(() => buildJBrowseShareUrl(
      { origin: 'https://seqedge.example', pathname: '/genomes/GCA_1' },
